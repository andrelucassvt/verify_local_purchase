...
5. **Contrato fechado no arquivo** — se a parte abre um contrato (interface, schema, rota) que uma parte posterior consome, esse contrato está especificado no próprio arquivo da parte, na mesma linha de contrato do `plan-template.md` (`produz ... · consome ...`), não fica para negociar durante a execução. Como a execução é sequencial, a parte seguinte só começa após o checkpoint.
...
O motivo curto é obrigatório em ambos os casos — força a avaliação explícita dos critérios acima em vez de um `sim`/`não` mecânico. Quando `sim`, acrescente o sufixo `· nível: mecânico | padrão` — mecânico só se a parte for transcrição de contrato já fechado com verificação binária; padrão nos demais casos.
